        ...
        switch (s[i])
        {
            case '\\':
                if (s[i+1] == 'n')
                {
                    ...
                    t[j] = s[i]; // We could probably be clever here and nest the breaks inside the else-if clauses, letting anything
                    i++;         // not matching \n or \t fall through and be treated as a normal character, but that would be less clear.
                    j++;         
                    t[j] = s[i];
                    i++;
                    j++;
